...
ENT.HasGrenadeAttack = false -- Chance that it will throw the grenade | Set to 1 to throw all the time
...
function ENT:PreInit()
	self.Model = "models/hl2_consul.mdl"
...
function ENT:CustomOnThink()
	if self:IsOnFire() then
		self.Bleeds = false
		timer.Simple(2, function() if self:IsValid() && self:IsOnFire() then self:TakeDamage(self:GetMaxHealth(), self, self) end end)
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
