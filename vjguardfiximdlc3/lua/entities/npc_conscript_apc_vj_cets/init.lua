...
ENT.DeathCorpseModel = "models/props_vehicles/apc001.mdl"
ENT.DeathCorpseCollisionType = COLLISION_GROUP_INTERACTIVE
ENT.EntitiesToNoCollide = {"npc_manhack", "npc_conscript_apc_vj_cets"}
...
function ENT:CustomInitialize_CustomTank()
	...
	self:SetCollisionBounds(Vector(130, 130, 63), Vector(-130, -130, -90))
	...
	self.bulletprop1:SetPos(self:GetAttachment(29).Pos)
	self.bulletprop1:SetParent(self,29)
...
function ENT:CustomOnThink()
	...
	if !self.APCDriveSound then
		self.APCDriveSound = CreateSound(self,"hl1/ambience/tankdrivein1.wav")
...
function ENT:CustomOnThink_AIEnabled()
	...
	if !self.VJ_IsBeingControlled then
		...
		local fire_dir = ( enemy:GetPos() - self:GetAttachment(1).Pos ):GetNormalized()
 		local localang = self:WorldToLocalAngles(fire_dir:Angle() + Angle(0,-40,0))
...
function ENT:CreateWheels()
	...
	{
		ent = self.Wheel_FR,
		pos = phys:GetPos()+self:GetForward()*45-self:GetRight()*68+self:GetUp()*14,
	...
	{
		ent = self.Wheel_RR,
		pos = phys:GetPos()+self:GetForward()*45+self:GetRight()*78+self:GetUp()*14,
		ang = phys:GetAngles()+Angle(0,90,90),
        },
	{
		ent = self.Wheel_RL,
...
function ENT:APC_stop()
	...
	if self.APCDriveSound:IsPlaying() then
		...
		self:EmitSound("hl1/plats/squeekstop1.wav",80,math.random(90, 110))
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:AimGun()
	...
	local pos = self.ShootPos or self:GetAttachment(29).Pos+self:GetRight()*-100
	local bullet_source = self:GetAttachment(29).Pos
...
function ENT:ShootBullet()
	...
	expLight:Fire("Color", "255 75 0")
	expLight:SetPos(bullet_source)
	expLight:Spawn()
	expLight:SetParent(self,30)
	expLight:Fire("TurnOn", "", 0)
	timer.Simple(0.1,function() if IsValid(expLight) then expLight:Remove() end end)
	self:DeleteOnRemove(expLight)
	ParticleEffectAttach("apc001_muzzleflash2_cets",PATTACH_POINT_FOLLOW,self,29)
	...
	self:EmitSound("hl1/ambience/mgun1.wav",90,95,1,CHAN_WEAPON)
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo, hitgroup, corpseEnt)
	...
	local gibphys = ents.Create("prop_physics")
		gibphys:SetPos(self:GetPos() + self:GetUp()*60)
		gibphys:SetModel("models/props_vehicles/apc001.mdl")
		gibphys:SetAngles(self:GetAngles())
		gibphys:Ignite(16)
		gibphys:Spawn()
		gibphys:Activate()

		local phys = gibphys:GetPhysicsObject()
		if IsValid(phys) then
			phys:Wake()

			local explosionPos = myPos
			local dir = (gibphys:WorldSpaceCenter() - explosionPos):GetNormalized()
	...
	for i = 1, 4 do
		...
		self.APCGib1:SetModel("models/props_vehicles/apc_tire001.mdl")
