...
SWEP.DrawWeaponInfoBox	= false
SWEP.BounceWeaponIcon = false 	
SWEP.ViewModel = "models/weapons/c_hl2_m16.mdl"
SWEP.WorldModel = "models/weapons/w_hl2_m16.mdl"
...
--------------------------------------------------------------------------------|
SWEP.Primary.Damage				= GetConVar("sk_plr_cets_dmg_m16"):GetInt() -- Damage
SWEP.Primary.ClipSize			= GetConVar("sk_max_cets_m16"):GetInt() -- Max amount of bullets per clip
SWEP.Primary.Delay				= 0.09 -- Time until it can shoot again
SWEP.Primary.Sound				= "Cets_Weapon_M16.Fire"
SWEP.Primary.TakeAmmo 			= 1
SWEP.Primary.Recoil				= .2
SWEP.Primary.Spread 			= .55   
SWEP.Primary.DefaultClip		= GetConVar("sk_max_cets_m16_bullet"):GetInt()
SWEP.Primary.NumberofShots 		= 1  
SWEP.Primary.Force 				= 1
SWEP.Primary.Automatic   		= true
SWEP.Primary.BulletShot 		= true
--------------------------------------------------------------------------------|
SWEP.Secondary.Sound = "Cets_Weapon_MP5SD.S_Fire"
SWEP.Secondary.ClipSize = -1
SWEP.Secondary.DefaultClip = 0
SWEP.Secondary.MaxAmmo = 10
SWEP.Secondary.Automatic = true
SWEP.Secondary.Ammo = "MP5Gr_CETS"
SWEP.Secondary.TakeAmmo = 1
SWEP.Secondary.Delay = 1
SWEP.Secondary.Recoil				= 0
SWEP.Secondary.Force = 0
--------------------------------------------------------------------------------|
SWEP.NPC_NextPrimaryFire = 2.4
SWEP.NPC_TimeUntilFire = 0.1
SWEP.NPC_TimeUntilFireExtraTimers = {0.2, 0.3, 0.4, 0.7, 0.8, 0.9, 1, 1.1}
...
function SWEP:PrimaryAttack(UseAlt)
	...
	-- Firing Sounds
	if SERVER then
		local fireSd = VJ.PICK("Cets_Weapon_M16.Fire")
	...
	local bullet = {} 
		...
		bullet.Spread = Vector( 0.182 * 0.182 , 0.182 * 0.182, 0)
                //The above, sets how far the bullets spread from each other. 
		bullet.Tracer = 1
		bullet.Force = self.Primary.Force 
	...
	if isPly then
		effect:SetOrigin(self:GetOwner():GetShootPos() - self:GetUp() * 6 + self:GetForward() * 22 + self:GetRight() * 10)
	elseif isNPC then
...
function SWEP:SecondaryAttack()
	...
	if self.Weapon:Ammo2() <= 0 then
		self.Weapon:EmitSound("hl1/weapons/dryfire1.wav")
		self:SetNextPrimaryFire( CurTime() + 0.2 )
		self:SetNextSecondaryFire( CurTime() + 0.2 )
	end
	if self.FiresUnderwater == false and self.Owner:WaterLevel() == 3 then
	...
	if SERVER then
		local entity = ents.Create( "obj_vj_cets_grenade_mp5" )
...
function SWEP:NPC_SecondaryFire()
	...
	local spawnPos = self:GetBulletPos()
	local projectile = ents.Create("obj_vj_cets_grenade_mp5")
...
if CLIENT then
	function SWEP:CreateWeaponSelectionFonts(height)
		...
		surface.CreateFont("CETS_M16font_Glow", {
			font = "CETS",
			size = math.min(27*scale, 150), --165
			weight = 500,
			antialias = true,
			additive = true,
			blursize = 5*scale,
		...
		surface.CreateFont("CETS_M16font", {
			font = "CETS",
			size = math.min(28*scale, 150), --150
			weight = 500,
			antialias = true,
			additive = true
		})
	...
	SWEP.DrawWeaponSelection = function(self, x, y, w, h, alpha)
		...
		draw.SimpleText(icon, "CETS_M16font_Glow", cx, cy, Color(r, g, b, glowAlpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.SimpleText(icon, "CETS_M16font", cx, cy, Color(r, g, b, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end