...
ENT.StartHealth = GetConVar("sk_cets_gargantua_baby_health"):GetInt()
ENT.SightAngle = 90
...
ENT.MeleeAttackDamage = GetConVar("sk_cets_gargantua_baby_dmg"):GetInt()
ENT.HasMeleeAttackKnockBack = true -- If true, it will cause a knockback to its enemy
ENT.MeleeAttackKnockBack_Forward1 = 270 -- How far it will push you forward | First in math.random
ENT.MeleeAttackKnockBack_Forward2 = 420 -- How far it will push you forward | Second in math.random
ENT.MeleeAttackDistance = 92
ENT.MeleeAttackDamageDistance = 97
...
ENT.FlinchSoundLevel = 90
ENT.MeleeAttackExtraSoundLevel = 80

ENT.MainSoundPitch = 150
ENT.MeleeAttackMissSoundPitch = 100

ENT.SoundTbl_FootStep = {"npc/gargantua/gar_step3.wav", "npc/gargantua/gar_step4.wav"}
...
ENT.SoundTbl_MeleeAttackExtra = "npc/gargantua/gar_shove2.wav"
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:Init()
	...
	self.DynamicLight:SetKeyValue("distance", "60")
	...
	self:SetCollisionBounds(Vector(35, 35, 320),  Vector(-35, -35, 0))
	...
	if self.Garg_CanFlame && self.Garg_NextFlameT < CurTime() && self.AttackType == VJ.ATTACK_TYPE_NONE then
		...
		if GetConVar("npc_cets_gargantua_preparation"):GetInt() == 1 then
			if self.Garg_FlameLevel == 0 then
				...
				self.Garg_NextFlameT = CurTime() + 0.4 -- Don't use anim duration because we want it to start playing the flame animation mid way
				timer.Simple(0.5, function() -- Play flame start sound
					if IsValid(self) && self.Garg_CanFlame then
						VJ.EmitSound(self, "npc/gargantua/gar_flameon1.wav", 80, 120)
					end
				end)
				return
			end
		...
		local range = 140
		VJ.ApplyRadiusDamage(self, self, self:GetPos() + self:OBBCenter() + self:GetForward()*15, range, GetConVar("sk_gargantua_dmg_fire"):GetInt() - 4, DMG_BURN, true, true, {UseConeDegree = 20}, function(ent) if !ent:IsOnFire() && (ent:IsPlayer() or ent:IsNPC()) then ent:Ignite(1) end end)
		
		self.Garg_FlameSd = VJ.CreateSound(self, "npc/gargantua/gar_flamerun1.wav", 80, 120)
		self:StopParticles()
		ParticleEffectAttach("ta_cremator_flamethrower_b", PATTACH_POINT_FOLLOW, self, 2)
		ParticleEffectAttach("ta_cremator_flamethrower_b", PATTACH_POINT_FOLLOW, self, 3)
...
function ENT:Garg_ResetFlame()
	...
	if self.Garg_FlameLevel == 2 then
		VJ.EmitSound(self, "npc/gargantua/gar_flameoff1.wav", 80, 120)
	end
...
function ENT:HandleGibOnDeath(dmginfo, hitgroup)
	...
	VJ.STOPSOUND(self.Garg_FlameSd)
	VJ.EmitSound(self, "weapons/explode" .. math.random(3, 5) .. ".wav", 150, 130)
	VJ.EmitSound(self, "npc/gargantua/gar_die" .. math.random(1, 2) .. ".wav", 150, 150)
...
function ENT:CustomOnKilled(dmginfo,hitgroup)
	...
	util.BlastDamage(self,self,self:GetPos(),400,100,DMG_BLAST,true,true)
	util.ScreenShake(self:GetPos(),22,500,1,500)
	...
	ParticleEffect("gonarch_explode_g", self:GetPos(), Angle(0,0,0), nil)
	ParticleEffect("gonarch_explode_i_b", self:GetPos(), Angle(0,0,0), nil)
	ParticleEffect("gonarch_explode_l", self:GetPos(), Angle(0,0,0), nil)
