...
function ENT:CustomOnInitialize()
	if self:WaterLevel() > 0 then 
		self:SetNoDraw(true)
		ParticleEffectAttach("acid_trails_bullsquid_water",PATTACH_ABSORIGIN_FOLLOW,self,0)
		...
		ParticleEffectAttach("acid_trails_bullsquid",PATTACH_ABSORIGIN_FOLLOW,self,0)
		self:SetAngles(self:GetVelocity():GetNormal():Angle())
		self:SetNoDraw(true)
