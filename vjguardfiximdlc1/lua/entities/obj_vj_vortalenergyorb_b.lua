...
ENT.MainSoundPitch = VJ.SET(65, 175)
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:Init()
	self.Vort_Lifetime = CurTime() + math.Rand(5, 10)
	self:SetSpawnEffect(true)
	self:DrawShadow(false)
	self:SetNoDraw(true)
	ParticleEffectAttach("electricball_4",PATTACH_ABSORIGIN_FOLLOW,self,0)
	...
	util.SpriteTrail(self, 0, colorWhite, true, 30, 0, 0.1, 1 / 6 * 0.5, "sprites/Vortal/orb_trail_1.vmt")
...
function ENT:OnThink()
	...
	if IsValid(trackedEnt) && self.Tracking == 1 then -- Homing Behavior
		...
		if IsValid(phys) then
			phys:SetVelocity(VJ.CalculateTrajectory(self, trackedEnt, "Line", self:GetPos(), self.Track_Position, 500))
		end
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:DeathEffects(data,phys)
	effects.BeamRingPoint(self:GetPos() +Vector(0, 0, 5), 0.2, 1, 400, 16, 3, Color(255, 128, 0, 64))
end