...
ENT.RadiusDamageType = bit.bor(DMG_NEVERGIB, DMG_SHOCK)
...
function ENT:CustomOnInitialize()
	...
	light:SetKeyValue( "rendercolor","128 255 86" )
	...
	self.DynLight:Fire("Color", "128 255 86")
...
function ENT:DeathEffects(data)
	effects.BeamRingPoint( self:GetPos(), 0.2, 0, self.RadiusDamageRadius*1.25, 10, 3, Color(128,255,86) )
	...
	for _,ent in pairs( ents.FindInSphere(self:GetPos(), self.RadiusDamageRadius) ) do
		if self:IsAlly(ent) && ent:GetClass() != "npc_vj_vortigaunt_synth" && ent != self then
