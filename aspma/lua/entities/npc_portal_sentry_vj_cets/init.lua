...
-----------------------------------------------*/
ENT.Model = "models/props/turret_01.mdl"
...
ENT.RangeAttackAngleRadius = 312
...
function ENT:OnThinkActive()
	...
	if math.abs(localang.r) > 10 && math.abs(localang.r) < 300 && self.IsGoingDown == 0 then
...
function ENT:CustomOnKilled()
	VJ.EmitSound(self, sdKillMe, 70, 100)
	local phys = self:GetPhysicsObject()
	local angles = self:GetAngles()
	...
	if self.Turret_Down == 1 then
		...
		self.bulletprop1:SetModel("models/props/turret_01.mdl")
		self.bulletprop1:SetPos(mypos)
		self.bulletprop1:SetSkin(1)
