...
ENT.Model = {"models/weapons/assassin_flechette.mdl"}
...
function ENT:CustomOnInitialize()
	...
	ParticleEffectAttach("assassin_flechette_trail",PATTACH_ABSORIGIN_FOLLOW,self,0)

	self.grenadeprop = ents.Create("prop_dynamic")
	self.grenadeprop:SetModel("models/blackout.mdl")
...
function ENT:Blink()
	...
	self:EmitSound("buttons/button17.wav",100,110)
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:Explode(deploy_hopwire)
	...
	timer.Simple(time_until_explode, function() if IsValid(self) then
		...
		if deploy_hopwire then
			local hopwire = ents.Create("obj_vj_hopwire")
			...
			effects.BeamRingPoint(  self:GetPos(), 0.35, 0, blastradius*2, 13, 13, Color(0,255,75) )
