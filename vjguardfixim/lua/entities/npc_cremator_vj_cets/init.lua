...
-----------------------------------------------*/
ENT.Model = {"models/cremator/cremator.mdl"}
ENT.VJ_NPC_Class = {"CLASS_COMBINE"}
ENT.StartHealth = GetConVar("sk_cremator_health"):GetInt()
---------------------------------------------------------------------------------------------------------------------------------------------
ENT.BloodColor = "Oil"
...
ENT.NextFlinchTime = 10 -- How much time until it can flinch again?
ENT.AnimTbl_Flinch = {
	"flinch", -- f l i c h
}
...
ENT.BeamDamage = GetConVar("sk_cremator_cremate_damage"):GetInt()
...
ENT.SoundTbl_Alert = {"npc/cremator/alert_player.wav"}
ENT.SoundTbl_CombatIdle = {"npc/cremator/amb_mad.wav"}
...
function ENT:FireGun()
	...
	util.ParticleTracerEx("cremator_beam", source, tr_main.HitPos, false, self:EntIndex(), 1)
	ParticleEffect("cremator_glow_cets",self:GetAttachment(1).Pos,Angle(0,0,0))

	local hitents = util.VJ_SphereDamage(self,self,tr_main.HitPos,self.BeamBlastRadius,self.BeamDamage,bit.bor(DMG_ENERGYBEAM, DMG_DISSOLVE, DMG_SHOCK, DMG_NEVERGIB),true,true,false,false)
		for _,ent in pairs(hitents) do
		if ent.IsVJBaseCorpse or ent:GetClass() == "prop_ragdoll" then
		...
		if ent:IsSolid() then
			if math.random(1,20) == 1 then
				VJ.EmitSound(self, "npc/cremator/plasma_ignite.wav", 80, math.random(100, 150))
			end
		end
	end
	...
	for i = 1,4 do
		local tr = util.TraceLine({
			...
			endpos = tr_main.HitPos + VectorRand(-128,128),
			mask = MASK_SHOT,
			filter = {self,self.Arc_Ent},
	})

	self.Arc_Ent:SetPos(tr_main.HitPos)
	util.ParticleTracerEx("cremator_beam", tr_main.HitPos, tr.HitPos, false, self.Arc_Ent:EntIndex(), 0)
...
function ENT:DoCremateAttack(swing_gun_anim,is_enemy_attack)
	...
	local anim = {"rangeattack2", "rangeattack1"}
	local duration = GetConVar("sk_cremator_cremate_dur"):GetInt()

	if swing_gun_anim then
		self.DoingGunSwing = true
		anim = "scriptedattack1"
	end
...
function ENT:StopCremateAttack(use_cooldown)
	...
	self:VJ_ACT_PLAYACTIVITY("endattack",true,0.9,false)
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:SetUpGibesOnDeath(dmginfo,hitgroup)
	...
	self:CreateGibEntity("prop_physics", "models/gibs/airboat_broken_engine.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 60)), Ang=self:GetAngles()+Angle(0, -90, 0)})
	self:CreateGibEntity("prop_physics", "models/gibs/manhack_gib03.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 55)), Ang=self:GetAngles()+Angle(0, -90, 0), Vel=self:GetRight()*math.Rand(150, 250)+self:GetForward()*math.Rand(-200, 200)})
	self:CreateGibEntity("prop_physics", "models/gibs/manhack_gib03.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 3, 55)), Ang=self:GetAngles()+Angle(0, -90, 0), Vel=self:GetRight()*math.Rand(-150, -250)+self:GetForward()*math.Rand(-200, 200)})
	self:CreateGibEntity("prop_physics", "models/gibs/manhack_gib03.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 55)), Ang=self:GetAngles()+Angle(0, -90, 0), Vel=self:GetRight()*math.Rand(150, 250)+self:GetForward()*math.Rand(-200, 200)})
	self:CreateGibEntity("prop_physics", "models/gibs/manhack_gib03.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 3, 55)), Ang=self:GetAngles()+Angle(0, -90, 0), Vel=self:GetRight()*math.Rand(-150, -250)+self:GetForward()*math.Rand(-200, 200)})
	self:CreateGibEntity("prop_physics", "models/gibs/manhack_gib03.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 55)), Ang=self:GetAngles()+Angle(0, -90, 0), Vel=self:GetRight()*math.Rand(150, 250)+self:GetForward()*math.Rand(-200, 200)})
	self:CreateGibEntity("prop_physics", "models/gibs/manhack_gib03.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 3, 55)), Ang=self:GetAngles()+Angle(0, -90, 0), Vel=self:GetRight()*math.Rand(-150, -250)+self:GetForward()*math.Rand(-200, 200)})
	self:CreateGibEntity("prop_physics", "models/props_wasteland/gear01.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 40))})
	self:CreateGibEntity("prop_physics", "models/props_wasteland/gear01.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 41))})
	self:CreateGibEntity("prop_physics", "models/cremator/cremator_head.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 42))})
	self:CreateGibEntity("prop_physics", "models/props_wasteland/gear02.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 35))})
	self:CreateGibEntity("prop_physics", "models/props_wasteland/gear02.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 35))})
	self:CreateGibEntity("prop_physics", "models/props_canal/winch02c.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 0))})
	self:CreateGibEntity("prop_physics", "models/props_canal/winch02c.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 0))})
	self:CreateGibEntity("prop_physics", "models/props_canal/winch02c.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 0))})
	self:CreateGibEntity("prop_physics", "models/props_canal/winch02c.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 0))})
	self:CreateGibEntity("prop_physics", "models/weapons/w_immolator.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 30))})
    for i = 1, 2 do
    end
    return true
end
...
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo, hitgroup, corpseEnt)
	...
	self:CreateGibEntity("physics_prop", "models/weapons/w_immolator.mdl", {CollisionDecal=false, Pos=self:LocalToWorld(Vector(0, 0, 30))})
end
---------------------------------------------------------------------------------------------------------------------------------------------
