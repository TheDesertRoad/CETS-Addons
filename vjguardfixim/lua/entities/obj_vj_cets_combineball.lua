...
ENT.DirectDamage = GetConVar("sk_cets_ar2scrotums_dirdam"):GetInt()
...
ENT.IdleSoundPitch = VJ.SET(95, 105)
---------------------------------------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------------------------------------------------
local colorWhite = Color(255, 255, 255, 64)
--
function ENT:Init()
	...
	if IsValid(owner) && owner:IsPlayer() then
		self.DirectDamage = GetConVar("sk_cets_ar2scrotums_dirdam_ply"):GetInt()
	end

	ParticleEffectAttach("combineball", PATTACH_POINT_FOLLOW, self, 1)
	util.SpriteTrail(self, 0, colorWhite, true, 1, 0, 0.1, 1 / 6 * 0.5, "sprites/combineball_trail_black_1.vmt")
...
function ENT:OnBounce(data, phys)
	...
	if target then
		...
		if self:GetForward():Dot(norm) < 0.55 then -- Lowered the visual range from 0.95, too accurate
...
function ENT:OnCollision(data, phys)
	VJ.EmitSound(self, "weapons/physcannon/energy_bounce" .. math.random(1, 2) .. ".wav", 80, 100)
	local owner = self:GetOwner()
...
function ENT:OnDestroy(data, phys)
	...
	VJ.ApplyRadiusDamage(self, self, myPos, 400, GetConVar("sk_cets_ar2scrotums_dieraddam"):GetInt(), bit.bor(DMG_SONIC, DMG_BLAST), true, true, {DisableVisibilityCheck=true, Force=80})
end