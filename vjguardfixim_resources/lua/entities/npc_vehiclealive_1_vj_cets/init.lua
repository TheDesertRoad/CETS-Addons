...
-----------------------------------------------*/
ENT.Model = "models/vehicles/CETS/cets_css_car.mdl"
...
ENT.SoundTbl_Breath = {
	"vehicles/cets/atv_idle1.wav",
	"vehicles/cets/atv_idle2.wav",
	"vehicles/cets/atv_idle3.wav",
	"vehicles/cets/atv_idle4.wav",
}

ENT.APC_RunOverDist = 350
ENT.APC_MinDriveDist = 1250
ENT.APC_AccelSpeed = 20
ENT.APC_DrivingSpeed = 300
ENT.APC_BackPedalSpeed = 200
ENT.APC_TurnSpeedMax = 50
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomInitialize_CustomTank()
	...
	self.APCAlarm = CreateSound(self, "vehicles/cets/honk1.wav")
...
function ENT:CustomOnThink()
	...
	if !self.APCDriveSound then
		self.APCDriveSound = CreateSound(self,"vehicles/cets/atv_secondgear1.wav")
...
function ENT:DriveThink()
	...
	if self:Visible(enemy) && (self:EnemyInDriveDist() or enemy_outside_of_rocket_ang) /*&& !flying_enemy_unreachable*/ then
		...
		VJ.EmitSound(self, "vehicles/cets/vehicle_turbo_loop3.wav", 100, 100)
		timer.Simple(3, function() if IsValid(self) then self:StopSound("vehicles/cets/vehicle_turbo_loop3.wav") end end)
	...
	local suspension = {
		{self.Wheel_FL, "vehicle_wheel_fl_height"},
		{self.Wheel_FR, "vehicle_wheel_fr_height"},
		{self.Wheel_RL, "vehicle_wheel_rl_height"},
		{self.Wheel_RR, "vehicle_wheel_rr_height"},
	}

	for _, v in ipairs(suspension) do
		local wheel = v[1]
		if !IsValid(wheel) then continue end

		local chassisPos = self:WorldToLocal(wheel:GetPos())
		local offset = math.Clamp(chassisPos.z / 14, -1, 1)

		self:SetPoseParameter(v[2], offset)
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CreateWheels()
	...
	{
		ent = self.Wheel_FR,
		pos = phys:GetPos()+self:GetForward()*45-self:GetRight()*68+self:GetUp()*20,
	...
	{
		ent = self.Wheel_RR,
		pos = phys:GetPos()+self:GetForward()*45+self:GetRight()*78+self:GetUp()*20,
		ang = phys:GetAngles()+Angle(0,90,90),
        },
	{
		ent = self.Wheel_RL,
...
function ENT:APC_stop()
	...
	if self.APCDriveSound:IsPlaying() then
		...
		self:EmitSound("vehicles/cets/atv_skid_highfriction.wav",90,math.random(90, 110))
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
