...
hook.Add("Think", "NV_Think", function()
	...
	if not ply:GetNWBool("HasNV", false) then
		keyWasDown = false
		oldNVState = false
		return
	end
	...
	if gui.IsGameUIVisible() or vgui.GetKeyboardFocus() then
		keyWasDown = input.IsKeyDown(KEY_N)
		return
	end
...
---------------------------------------------------------------------------------------------------------------------------------------------
hook.Add("Think", "NV_DynamicLight", function()
	local ply = LocalPlayer()
	...
	if not ply:GetNWBool("NVActive", false) then
		return
	end

	local dlight = DynamicLight(987654)
	...
	dlight.pos = eyePos + eyeAngles:Forward() * 8
	dlight.r = 0
	dlight.g = 255
	dlight.b = 0
	dlight.brightness = 0
